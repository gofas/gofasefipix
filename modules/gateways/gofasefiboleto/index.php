	...
	function gefib_api_connect(){
		...
		if($params['sandbox']){
			$params_api = [
				...
				'charge_url' => 'https://cobrancas-h.api.efipay.com.br',
			];
		}
		if(!$params['sandbox']){
			$params_api = [
				...
				'charge_url' => 'https://cobrancas.api.efipay.com.br',
			];
		}
		return $params_api;
	}
    	...
    	if(stripos($_SERVER['REQUEST_URI'], 'viewinvoice') !== false ){
    		...
    		if($params['amount'] >= $params['minimunamount']){	
    			...
    			function copy_tooltip() {
    				var copyText = document.getElementById("billetforcopy");
    				copyText.select();
    			...
    			$saved_billet = gefib_get_local_qrc($params['invoiceid']);
    			
				$GetInvoiceResults			= localAPI('getinvoice',array('invoiceid'=>$params['invoiceid'] ), (int)gefib_setup_admin()['id'] );
    			
    			if($saved_billet['barcode'] and (float)$saved_billet['amount'] === (float)$params['amount'] and strtotime("now -1 hour") < strtotime($saved_billet['updated_at']) ){
    				$charge_verify = gefib_charge_verify($saved_billet['txid']);
    				$log['charge_verify'] = $charge_verify;
    				if((string)$charge['result']['status'] === (string)'CONCLUIDA'){
    					$add_trans = gefib_add_trans($params['clientdetails']['id'],$params['invoiceid'], (float)$charge['result']['valor']['original'], gefib_fee($charge['result']['valor']['original']), 'gefib-'.$params_api['api_mode'].'-'.$billet['txid'], 'Boleto pago - confirmação ao acessar a fatura');
						header_remove();
    					header("Location: ".gefib_whmcs_url('whmcs_url').'/viewinvoice.php?id='.$params['invoiceid'],true,303);
    					exit;
    				}
    				$result .= $params['message'];
					$result .= '<img style="width: 200px;border: 1px solid #ccc;" src="'.$saved_billet['billet_link'].'">';
    				$result .= '<input value="'.$saved_billet['barcode'].'" id="billetforcopy" style="width: 0px;height: 0px;font-size: 0px;padding: 0px;display:none;">';
    				...
    				if(!$error and $params['redirecttobillet'] and stripos($_SERVER['REQUEST_URI'], 'viewinvoice') ){
    					header_remove();
    					header("Location: ".$saved_billet['barcode'],true,303);
    					exit;
    				}
    				else {
    					return $result;
    			...
    			if(!$saved_billet['barcode'] || !$saved_billet['billet_link'] || (float)$saved_billet_amount !== (float)$params['amount']){
    				$line_items = array();
					...
					$postfields = [
						'items'=>[[
							'name'=>(string)(substr( implode(", ",$line_items),  0, 140)),
							'value'=>(int)preg_replace("/[^0-9]/", "",$params['amount']),
							'amount'=>1
						]],
						'payment'=>[
						  'banking_billet'=>[
						...
						'metadata'=>[  
    						...
    						'notification_url'=> gefib_whmcs_url('whmcs_url').'modules/gateways/gofasefiboleto/includes/callback.php',
						]
					];
					if($customer['cnpj']){
						$postfields = [
    				...
    				$billet_ = gefib_charge($postfields);
    				if((int)$billet_['result_code'] !== (int)200){
    					$error .= $billet_['result_code'].': ';
    					if(is_array($billet_['result']['data']['errors'])){
							foreach($billet_['result']['data']['errors'] as $key=>$value){
    				...
    				$log['billet_'] = $billet_;
    				if($billet_['result']['data']['barcode']){
                        if(!$saved_billet['barcode'] || !$saved_billet['billet_link']){
    						$save_qrc = gefib_save_qrc(
    							[
    								'invoice_id'=>$params['invoiceid'],
    								'id'=>$billet_['result']['data']['id'],
									'txid'=>$billet_['result']['data']['cob']['txid'],
    								'amount'=>$params['amount'],
    								'barcode'=>$billet_['result']['data']['barcode'],
    								'billet_link'=>$billet_['result']['data']['billet_link'],
    					...
    					if($saved_billet['barcode']){
    						$update_qrc = gefib_update_qrc(
    							[
    								...
    								'amount'=>$params['amount'],
    								'barcode'=>$billet_['result']['data']['barcode'],
    								'billet_link'=>$billet_['result']['data']['imagemQrcode'],
    								'api_mode'=>$params_api['api_mode'],
    							]
    						);
    					...
    					$result .= $params['message'];
						$result .= '<img style="width: 200px;border: 1px solid #ccc;" src="'.$billet_['result']['data']['imagemQrcode'].'">';
    					//$result .= '<a target="_blank" class="btn btn-default" style=" float: left;font-size: 14px;" href="'.$billet_['result']['data']['pix']['barcode'].'">Visualizar o Boleto</a>';
    					$result .= '<input value="'.$billet_['result']['data']['barcode'].'" id="billetforcopy" style="width: 0px;height: 0px;font-size: 0px;padding: 0px;display:none;">';
    					$result .= '<button style="position: relative;font-size: 14px; display: inline-block;width: 200px;"  id="copy_tooltip" class="btn btn-default" onclick="copy_tooltip()" onmouseout="outFunc()">Boleto Copia e Cola</button>';
    				}
    			}
    			if($error){
    			...
    			if(!$error and $params['redirecttobillet'] and stripos($_SERVER['REQUEST_URI'], 'viewinvoice') ){
    				header_remove();
    				header("Location: ".$billet_['result']['data']['pix']['barcode'],true,303);
    				exit;
    			}
    			else {
    				return $result;
...
if( !function_exists('gefib_get_token') ){
	function gefib_get_token(){
		$params_api = gefib_api_connect();
		$curl = curl_init($params_api['charge_url'].'/v1/authorize');
...
if(!function_exists('gefib_charge')){
	function gefib_charge($postfields){
		...
		curl_setopt_array($curl, [
			CURLOPT_URL => $params_api['charge_url'].'/v1/charge/one-step',
		...
		curl_close($curl);
		return ['access_token'=>$access_token,'result_code'=>$result_code,'result'=>$result];
	}
}
...
if(!function_exists('gefib_save_qrc')){
	function gefib_save_qrc($qr_code){
		$data = array(
			...
			'barcode'=>$qr_code['barcode'],
			'billet_link'=>$qr_code['billet_link'],
...
if($_REQUEST['invoice_id']){
	...
	if( $invoice['invoiceid']){
		$billet = gefib_get_local_qrc($_REQUEST['invoice_id']);	
		$charge = gefib_charge_verify($billet['txid']);
		if(((STRING)$charge['result']['status'] === (STRING)'CONCLUIDA') and $invoice['status'] !== 'Paid' and (float)$invoice['total'] === (float)$charge['result']['valor']['original']){
			$add_trans = gefib_add_trans($invoice['userid'],$_REQUEST['invoice_id'], (float)$charge['result']['valor']['original'], gefib_fee($charge['result']['valor']['original']), 'gefib-'.$params_api['api_mode'].'-'.$billet['txid'], 'Boleto pago - confirmação enquanto o cliente visualizava a fatura');			
		}
...
if($_POST['id']){
	...
	if( $invoice['invoiceid']){
		$billet = gefib_get_local_qrc($_POST['id']);	
		$charge = gefib_charge_verify($billet['txid']);
		if(((string)$charge['result']['status'] === (string)'CONCLUIDA') and $invoice['status'] !== 'Paid' and (float)$invoice['total'] === (float)$charge['result']['valor']['original']){
			$add_trans = gefib_add_trans($invoice['userid'],$_POST['id'], (float)$charge['result']['valor']['original'], gefib_fee($charge['result']['valor']['original']), 'gefib-'.$params_api['api_mode'].'-'.$billet['txid'], 'Boleto pago - confirmação via webook /viewinvoice.php');			
		}
		if($charge['result']['status']){
			echo $charge['result']['status'];
		}
